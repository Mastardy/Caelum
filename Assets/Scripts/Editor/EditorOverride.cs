...
namespace Pixeye.Unity
{
		...
		public override void OnInspectorGUI()
		{
			...
			void Foldout(CacheFoldProp cache)
			{
				...
				if (cache.expanded)
				{
					...
					{
						var i1 = i;
						this.UseVerticalLayout(() => Child(i1), StyleFramework.boxChild);
					}
				}
			...
			void Setup()
			{
				...
				if (!initialized)
				{
					...
					for (var i = 0; i < length; i++)
					{
						...
						if (fold == null)
						{
							if (prevFold is {foldEverything: true})
							{
	...
	static class StyleFramework
	{
		...
		static StyleFramework()
		{
			...
			{
				font = Font.CreateDynamicFontFromOSFont(new[] {"Terminus (TTF) for Windows", "Calibri"}, 17)
			};
			...
			{
				overflow = new RectOffset(25, 0, 3, 0),
